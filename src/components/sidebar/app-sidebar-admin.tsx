...
export function AppSidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex md:flex-col w-64 bg-cream shadow-md rounded-md h-screen p-4 fixed top-0 left-0 justify-between">
        <div>
          <Image
            src={Logo}
            alt="Logo"
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-cream shadow-md flex justify-around items-center p-2 md:hidden z-50">
        {items.map((item) => (
          <button
            key={item.title}
            onClick={() => handleNavigation(item.url)}
